...
public class PlayerListCell extends ListCell<String> {
    ...
    EventHandler<ActionEvent> showPlayerDetail;
    EventHandler<ActionEvent> kickPlayer;
    EventHandler<ActionEvent> banPlayer;

    public PlayerListCell(ListView<String> listView, PlayerInfoRepository playerInfoRepository,
                          FlyableUnitService flyableUnitService) {

        this.playerInfoRepository = playerInfoRepository;
        this.flyableUnitService = flyableUnitService;

        this.showPlayerDetail = event -> {
            // show a prompt dialog that contains the info for this player
            PlayerInfo playerInfo = playerInfoRepository.findByName(getItem()).orElseThrow(
            ...
            String unitName =
                    playerInfo.getSlot().equals("") ?
                            "" :
                            "[" +
                                    flyableUnitService.findByUnitId(playerInfo.getSlot()).orElse(new FlyableUnit()).getUnit_name() +
                                    "]";
