...
public interface AdminInfoDao extends BaseMapper<AdminInfo> {
    ...
    @Select("SELECT role_info.name roles FROM group_role,role_info WHERE group_role.role_id = role_info.id AND " +
    ...
    @Select("SELECT group_info.name groups FROM user_group,group_info WHERE user_group.group_id = group_info.id " +
